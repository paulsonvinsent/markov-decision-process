...
import edu.gatech.cs7641.assignment4.artifacts.*;
...
public class Experiments {
     ...
     * (so no ValueIterationAlgorithm or PolicyIterationAlgorithm). It is somewhat redundant to the specific analysis
     ...
     * by setting this constant to true, you'll be running the QLearningAlgorithm experiment twice (so double
     * the time).
     */
    private static boolean USE_LEARNING_EXPERIMENTER = false;

    public static Analysis runAlgorithmAndGetResults(AlgorithmDescription algorithm, Problem problem) {
        ...
        switch (algorithm.type) {
            case ValueIteration:
                ValueIterationAlgorithm vi = (ValueIterationAlgorithm) algorithm;
                runAlgorithm(analysis, problem, domain, hashingFactory, initialState, (episodeIndex, domain1, hashingFactory1, simulatedEnvironment) -> new ValueIteration(domain1, vi.gamma, hashingFactory1, vi.maxDelta, episodeIndex), algorithm.type);
                break;
            case PolicyIteration:
                PolicyIterationAlgorithm pi = (PolicyIterationAlgorithm) algorithm;
                runAlgorithm(analysis, problem, domain, hashingFactory, initialState, new PlannerFactory() {
                    ...
                    public Planner createPlanner(int episodeIndex, SADomain domain, HashableStateFactory hashingFactory, SimulatedEnvironment simulatedEnvironment) {
                        ...
                        /*
                         * For PolicyIterationAlgorithm we need to specify the number of iterations of
                         * ValueIterationAlgorithm that the algorithm will use internally to compute the
                         * corresponding values. By default, the code is using the same number of
                         * iterations specified for the ValueIterationAlgorithm algorithm.
                         *
                         * A recommended modification is to change this value to the actual number
                         * of iterations that it takes ValueIterationAlgorithm to converge. This will
                         * considerably improve the runtime of the algorithm (assuming that
                         * ValueIterationAlgorithm converges faster than the number of configured
                         * iterations).
                         */
                        return new PolicyIteration(domain, pi.gamma, hashingFactory, pi.maxDelta, pi.numberOfInteralIterations, episodeIndex);
                    }
                }, algorithm.type);
                break;
            default:
                QLearningAlgorithm ql = (QLearningAlgorithm) algorithm;
                runAlgorithm(analysis, problem, domain, hashingFactory, initialState, new PlannerFactory() {

                    @Override
                    public Planner createPlanner(int episodeIndex, SADomain domain, HashableStateFactory hashingFactory, SimulatedEnvironment simulatedEnvironment) {
                        QLearning agent = new QLearning(domain, ql.gamma, hashingFactory,
                                0.1, ql.learningRate, ql.learningPolicy, Integer.MAX_VALUE);
    ...
    private static Problem createProblem1() {
        ...
        hazardRewardsHashMap.put(HazardType.MEDIUM, -10.0);
         ...
         * is happy).
         */
        return new Problem("Easy Grid", map, numIterationsHashMap, -0.1, 10, hazardRewardsHashMap);
    }
    ...
    private static Problem createProblem2() {
        ...
        return new Problem("Hard Grid", map, numIterationsHashMap, -0.1, 10, hazardRewardsHashMap);
    }
